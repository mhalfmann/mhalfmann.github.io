...
class LocalLogger {
    ...
    registerUser(userid, password, email, group, browserinfo, screeninfo) {
        ...
        request.onsuccess = function (evt) {
            ...
            if (cursor) {
                ...
                let dbitem = { userid: userid, passwordhash: passwordhash, email: email, roles: roles, userguid: self.generateUUID(), creationtime: Date.now(), group: group, browserinfo: browserinfo, screeninfo: screeninfo }
    ...
    getOpenRun(initialdata) {
        ...
        request.onsuccess = function (evt) {
            ...
            if (cursor) {
                let r = cursor.value
                if (condstats[r.conditionid] != null) {
                    condstats[r.conditionid]++
                } else {
                    condstats[r.conditionid] = 1
                }
                cursor.continue()
            } else {
                let event = new CustomEvent("openrunresult", { detail: { runfound: false, runid: "0", conditionid: "", neededuser: 1, condstats: condstats } })
                window.dispatchEvent(event)
            }
        }
    }
            ...
            if (cursor) {
                ...
                if (ru.includes(self.userid)) {
                    ...
                    let event = new CustomEvent("getownrunresult", {
                        detail: {
                            runid: self.runid,
                            conditionid: cursor.value.conditionid,
                            neededuser: cursor.value.neededuser,
                            runuser: cursor.value.runuser,
                            runuserdata: cursor.value.runuserdata[self.userid]
                        }
                    })
                    window.dispatchEvent(event)
                } else {
    ...
    pageEntered(pagecounter, pageid, timestamp, pagehistory, backcounter, userrunstatus) {
        ...
        request.onsuccess = this.pageEnteredStep2.bind(this, pagecounter, pageid, timestamp, pagehistory, backcounter, userrunstatus)
    }

    pageEnteredStep2(pagecounter, pageid, timestamp, pagehistory, backcounter, userrunstatus, event) {
        ...
        if (cursor) {
            ...
            runuserdata[this.userid]["userrunstatus"] = userrunstatus //started, advanced, completed, aborted
            if (runuserdata[this.userid]["starttime"] == "") {
                runuserdata[this.userid]["starttime"] = timestamp
            }
            runuserdata[this.userid]["lastactivity"] = timestamp
            let runupdaterequest = cursor.update(dbrunitem)
            runupdaterequest.onsuccess = function () {
                console.log(`rundata updated`)
            }
            let dbitem = {
                studyid: this.studyid,
                runid: this.runid,
        ...
        request.onsuccess = function (evt) {
            ...
            if (cursor) {
                ...
                request2.onsuccess = function () {
                    console.log(`logging successfull ${pageid} pagedata ${JSON.stringify(data)}`)
                }
                ...
                request3.onsuccess = self.pageLeftStep2.bind(self, pagecounter, pageid, timestamp, data, pagemetadata, pagegroupmetadata, runtimedatachanged, runtimedata, errorlog, visittime)
            }
        }
    }

    pageLeftStep2(pagecounter, pageid, timestamp, data, pagemetadata, pagegroupmetadata, runtimedatachanged, runtimedata, errorlog, visittime, evt) {
        let cursor = evt.target.result
        if (cursor) {
            let item = cursor.value
            ...
            item.runuserdata[this.userid]["errorlog"] = errorlog
            let request2 = cursor.update(item)
    ...
    getPagevisitData(datafromuserid, formkey, callback) {
        ...
        let store = trans.objectStore("pagevisit")
        let index = store.index("pagevisit_index4")
        let request = index.openCursor(IDBKeyRange.only([this.studyid, this.runid, datafromuserid, datafrompageid]))
        let formvalue
