...
class Study {
    constructor() {
        ...
        this.meta.browserinfo = parser.getResult()
        ...
        this.errorprocessor = new ErrorProcessor(this.meta.errorlog)
        ...
        let upautologin = this.readURLParameter("autologin")
        if (upautologin == "true" || upautologin == "1") {
            upautologin = true
        } else {
            upautologin = false
        }
        let upuserid = this.readURLParameter("userid")
        let uppasswd = this.readURLParameter("passwd")
        let setupviewvisible
        if (upautologin == true && upuserid != "" && uppasswd != "") {
            this.meta.autologin = true
            this.meta.userid = upuserid
            this.meta.password = uppasswd
            setupviewvisible = false
        } else {
            setupviewvisible = IWM.globalconfig.setupviewvisible
        }
    ...
    loadStudyConfigFinished() {
        ...
        if (this.meta.appversion < this.meta.min_appversion) {
            alert(
                "Die Version " +
                    this.meta.appversion.toString() +
                    " der Versuchsumgebung ist nicht ausreichend für die Studie " +
                    this.meta.studyid +
                    ". Es wird mindestens die Version " +
                    this.meta.min_appversion.toString() +
                    " benoetigt. Bitte melden Sie sich beim Versuchsleiter."
            )
        }
        ...
        this.meta.register_visible = IWM.studyconfig.welcomepage.register_visible
        ...
        for (let i = 0; i < allcond.length; i++) {
            ...
            if (con.active) {
                ...
                this.meta.availableconditions.push({ conditionid: con.conditionid, neededuser: con.neededuser, desiredrunnumber: con.desiredrunnumber })
            }
        }
        if (this.meta.loginmode == "anonym") {
            ...
            if (this.meta.autologin == true) {
                loginuser.value = this.meta.userid
                loginpassword.value = this.meta.password
                this.loginUser()
            } else {
                //register
                if (!this.meta.register_visible) {
    ...
    registerUser() {
        let bi = this.meta.browserinfo
        bi = bi.browser.name + "|" + bi.browser.major + "|" + bi.os.name + "|" + bi.os.version + "|" + bi.device.type + "|" + bi.cpu.architecture
        let si = window.screen.width + "x" + window.screen.height + "|" + window.innerWidth + "x" + window.innerHeight
        if (this.meta.loginmode == "anonym") {
            ...
            this.logger.registerUser(this.meta.userid, this.meta.password, this.meta.email, this.meta.group, bi, si)
        } else if (this.meta.loginmode == "register") {
    ...
    registerUserSucceeded(e) {
        ...
        this.logger.getOpenRun(this.meta.initialdata)
    }
    ...
    processOpenRunResult(e) {
        if (e.detail.runfound) {
            ...
            if (!this.meta.conditionid) {
                if (e.detail.condstats != null) {
                    let condstats = e.detail.condstats
                    let countmin = 9999999999
                    let countmincond
                    let desiredrunnumberreached = []
                    for (let con of this.meta.availableconditions) {
                        if (condstats[con.conditionid] != null) {
                            con["currentcount"] = condstats[con.conditionid]
                        } else {
                            con["currentcount"] = 0
                        }
                        if (con["desiredrunnumber"] != null) {
                            if (con["currentcount"] >= con["desiredrunnumber"]) {
                                desiredrunnumberreached.push(con.conditionid)
                            }
                        }
                    }
                    for (let con of this.meta.availableconditions) {
                        if (desiredrunnumberreached.length != this.meta.availableconditions.length) {
                            if (con["currentcount"] <= countmin) {
                                if (con["desiredrunnumber"] == null || con["desiredrunnumber"] > con["currentcount"]) {
                                    countmin = con["currentcount"]
                                    countmincond = con
                                }
                            }
                        } else {
                    ...
                    let modcondpool = Array.from(this.meta.availableconditions)
                    if (desiredrunnumberreached.length != this.meta.availableconditions.length) {
                        let modcondpool2 = []
                        for (let con of modcondpool) {
                            if (desiredrunnumberreached.indexOf(con.conditionid) == -1) {
                                modcondpool2.push(con)
                            }
                        }
                        modcondpool = modcondpool2
                    }
                    if (countmincond != null) {
                        modcondpool.push(countmincond)
                        modcondpool.push(countmincond)
                    }
                    let r = this.createRandomIntFromInterval(0, modcondpool.length - 1)
                    this.meta.conditionid = modcondpool[r].conditionid
                    this.meta.neededuser = modcondpool[r].neededuser
                } else {
            ...
            this.logger.createRun(this.meta.conditionid, this.meta.neededuser, this.meta.expversion, this.meta.initialdata)
        }
    }
    ...
    continueOwnRun(e) {
        ...
        this.meta.errorlog = this.meta.continuedrunuserdata["errorlog"]
        this.meta.initialdata = this.meta.continuedrunuserdata["initialdata"]
    ...
    loadConditionConfigFinished() {
        ...
        if (this.meta.runcontinued && this.meta.continuedrunuserdata["currentpage"] != "") {
            this.flowcontrol.initFromRunUserData(IWM.conditionconfig, this.meta.continuedrunuserdata, this.meta.manualfirstpage)
        } else {
    ...
    loadedPage() {
        ...
        this.logger.pageEntered(this.flowcontrol.pagecounter, this.flowcontrol.currentpage.pageid, Date.now(), "pagehistory", this.flowcontrol.backcounter, this.flowcontrol.userrunstatus)
        this.sendMessageToStudyAddOn("initpage", {
            pageinfo: this.flowcontrol.currentpage.getPageinfoForAddon(),
            pagegroupinfo: this.flowcontrol.getPagegroupinfoForAddon(),
            studyinfo: this.getStudyinfoForAddon()
        })
    }

    initPageWithExistingData(e) {
        if (this.flowcontrol.currentpage.isreloaddata == false) {
            console.log(`init page data reload disabled`)
            this.sendMessageToStudyAddOn("initexistingdata", { data: null })
        } else {
    ...
    changePage(targetpageid, logdata, visittime, runtimedata) {
        ...
        this.logger.pageLeft(
            this.flowcontrol.pagecounter,
            this.flowcontrol.currentpage.pageid,
            Date.now(),
            logdata,
            this.flowcontrol.getAllPageMetadata(),
            this.flowcontrol.getAllPagegroupMetadata(),
            runtimedatachanged,
            rtd,
            this.meta.errorlog,
            visittime
    ...
    getStudyinfoForAddon() {
        ...
        return {
            studyid: this.meta.studyid,
            user: this.meta.userid,
            conditionid: this.meta.conditionid,
            runid: this.meta.runid,
            runuser: this.meta.runuser,
            otherusers: otherusers,
            pagecounter: this.flowcontrol.pagecounter,
            runtimedata: this.meta.runtimedata,
            apporigin: this.meta.apporigin,
            history: this.flowcontrol.history,
            initialdata: this.meta.initialdata,
            browserinfo: this.meta.browserinfo
        }
    }
}
