...
class StudyLocalManager {
    ...
    reportingCSVCompact() {
        ...
        this.data = [["study", "version", "condition", "run", "runcreated", "user", "group", "runtimedata"]]
        ...
        request.onsuccess = function (evt) {
            ...
            if (cursor) {
                ...
                for (let i = 0; i < rundata.runuser.split(",").length; i++) {
                    ...
                    self.data.push([rundata.studyid, rundata.expversion, rundata.conditionid, rundata.runid, rundata.creationtime, usr, self.allstudyuser.get(usr).group, JSON.stringify(rundata.runuserdata[usr].runtimedata)])
                }
                cursor.continue()
            }
            else {
    ...
    reportingCSVPagevisits() {
        ...
        request.onsuccess = function (evt) {
            ...
            if (cursor) {
                ...
                let visittime = 0
                if (dbitem.endtime) {
                    visittime = dbitem.endtime - dbitem.starttime
                }
                let dataset = [dbitem.studyid, dbitem.runid, dbitem.userid, dbitem.pagecounter, dbitem.pageid, dbitem.starttime, dbitem.endtime, visittime, JSON.stringify(dbitem.data)]
