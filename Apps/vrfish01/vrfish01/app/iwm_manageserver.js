...
class StudyServerManager {
    ...
    reportingCSVActionlog(event) {
        console.log("reporting csvactionlog requesting data")
        this.clearReporting()
    ...
    clearReporting() {
        ...
        document.getElementById("reporting_linkcsvactionlogsave").style.color = "#dddddd"
        document.getElementById("reporting_linkcsvactionlogsave").removeAttribute("href")
        document.getElementById("reporting_linkcsvactionlogsave").removeAttribute("download")
        document.getElementById("reporting_linkcsvactionlogshow").style.color = "#dddddd"
        document.getElementById("reporting_linkcsvactionlogshow").style.cursor = "text"
        document.getElementById("reporting_linkcsvactionlogshow").removeEventListener("click", this.showCSVViewBound)        
    ...
    selectData() {
        ...
        sel.removeAllRanges()
        sel.addRange(range)
        alert("Alle Daten wurden ausgewählt, auch wenn die Anzeige der Auswahl evtl. unvollständig ist. Zum Kopieren den markierten Bereich kurz berühren. Die Daten können in Numbers eingefügt werden.")
        target.readonly = true
            ...
            if (request.status == 0) {
                ...
                if (response.error == true) {
                    ...
                    if (response.action == "manage_login" && response.status == "success") {
                        ...
                        document.getElementById("login_lastlogin").value = ll.toISOString()
                        ...
                        for (let i = 0; i < response.data.length; i++) {
                            this.allsubjects.push(new Subject(response.data[i].userid, response.data[i].group, response.data[i].runid, response.data[i].condition, response.data[i].currentpage))
                        }
                        ...
                        this.reportingGenerateCSVLinks(response.csv, "actionlog")
                    }
                        ...
                        if(response.zipfilename != ""){
                            this.reportingGenerateZIPLink(response.zipfilename, response.uncompressedsize, response.ac)
                        }
                        else{
    ...
    reportingGenerateCSVLinks(csvcontent, mode) {
        ...
        if (navigator.msSaveBlob) { // IE 10+
            ...
            if (linksave.download !== undefined) { // feature detection
    ...
    reportingGenerateZIPLink(zipfilename, uncompressedsize, accesscode) {
        if(zipfilename == this.studyid + "_files.zip"){
            console.log("creating zip link")
            let estimatedwaitingtime = uncompressedsize/1000000 * 50
            document.getElementById("reporting_linkfilesdownload").innerHTML = Math.round(estimatedwaitingtime / 1000)
            this.zipfilename = zipfilename
            this.accesscode = accesscode
            this.currenttimeout = setInterval(this.reportingProcessZIPLinkStatus.bind(this),1000)
        }        
    }

    reportingProcessZIPLinkStatus(){
        let t = parseInt(document.getElementById("reporting_linkfilesdownload").innerHTML)
        if(t > 1 ){
            document.getElementById("reporting_linkfilesdownload").innerHTML = t - 1
        }
        else {
            clearInterval(this.currenttimeout)
            document.getElementById("reporting_linkfilesdownload").innerHTML = "herunterladen (30 Minuten verfügbar)"
            document.getElementById("reporting_linkfilesdownload").style.color = "#000000"
            document.getElementById("reporting_linkfilesdownload").setAttribute("href","https://versuche.iwm-tuebingen.de/download?file=" + this.zipfilename + "&ac=" + this.accesscode)
        }
    }      
    ...
    generateSubjectTableCells() {
        let result = "<tr><td>Gruppe</td><td>Benutzer-ID</td><td>Versuchslauf</td><td>Bedingung</td><td>aktuelle Seite</td><td>Passwort</td></tr>"
        for (let i = 0; i < this.allsubjects.length; i++) {
            ...
            row = row + "<td style='width:80px;'><button id='subjecteditbtn_" + subject.userid + "' class='button has-text-white has-background-grey' style='width:80px; height:25px; padding:0px;' onclick='iwmstudy_manager.subjectChangePassword(event)'>ändern</button></td></tr>"
            result = result + row
        }
        return result
    }
    ...
    subjectChangePasswordSend(event) {
        ...
        let requestdata = { action: "manage_subjectchangepassword", studyid: this.studyid, userid: this.userid, passwordhash: this.passwordhash, s_userid: s_userid, s_group: s_group, s_passwordhash: s_passwordhash }
        this.doRequest(requestdata)
        this.mode = "subject"
        this.changeView()
    }
    ...
    generateUserTableCells() {
        ...
        for (let i = 0; i < this.alluser.length; i++) {
            ...
            row = row + "<td style='width:220px;'>" + ll.toISOString() + "</td>"            
            row = row + "<td style='width:90px;'><button id='usereditbtn_" + user.userid + "' class='button has-text-white has-background-grey' style='width:90px; height:25px; padding:0px;' onclick='iwmstudy_manager.userEdit(event)'>bearbeiten</button></td></tr>"
            result = result + row
        }
        return result
    }
    ...
    userCreateSend(event) {
        ...
        let requestdata = { action: "manage_usercreate", studyid: this.studyid, userid: this.userid, passwordhash: this.passwordhash, c_userid: c_userid, c_passwordhash: c_passwordhash, c_role: c_role, c_studies: c_studies, c_groups: c_groups }
        this.doRequest(requestdata)
        this.mode = "user"
        this.changeView()
    }
    ...
    userEditSend(event) {
        ...
        let requestdata = { action: "manage_useredit", studyid: this.studyid, userid: this.userid, passwordhash: this.passwordhash, e_userid: e_userid, e_role: e_role, e_studies: e_studies, e_groups: e_groups }
        this.doRequest(requestdata)
        this.mode = "user"
        this.changeView()
    }
...
class Subject {
    constructor(userid, group, runid, condition, currentpage) {
        this.userid = userid
        this.group = group
        this.runid = runid
